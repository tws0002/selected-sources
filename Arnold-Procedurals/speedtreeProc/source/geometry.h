...
{
  FSpeedTreeData speedtreeData;
  FSpeedTreeData speedtreeMotionData;
  AtString srtFile;
  string meshName;
...
static void srt2AtArray(const SDrawCall& sDrawCall, SpeedTreeData* data)
{
    ...
    for (unsigned int i = 0; i < sDrawCall.m_nNumVertices; ++i)
    {
        ...
        if (data->enableMotionBlur)
        {
            vec3 offset_motion = GetSpeedTreeVertexOffset(vtxParam, data->procArrays[data->id]->GeometryType, data->WindType, data->LODSmoothType,
            ...
            AiArraySetVec(data->procArrays[data->id]->vlistArray, (i + sDrawCall.m_nNumVertices), tmp_m);
...
AtNode* createNode(const AtNode* parent, SpeedTreeData *data)
{
    ...
    // different shaders to different polymeshes in one procedural node.
